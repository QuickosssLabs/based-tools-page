...
async function payWithMetamask(priceInEth, toolName) {
    try {
        ...
        document.getElementById('transactionAmount').value = priceInEth;
...
document.querySelector('.submit-details').addEventListener('click', function(e) {
    ...
    console.log(`Name: ${name}, Email: ${email}, Collection Name: ${collectionName}, Amount: ${amount}, Transaction ID: ${transactionId}`);

    // Ensure all fields are filled
    if (!name || !collectionName || !collectionLink || !amount || !transactionId) {
        alert("Please fill in all required fields.");
        return;
    }

    // Send email notification
    sendEmailNotification(name, email, collectionName, collectionLink, collectionLogo, toolName, amount, transactionId);
});
